...
- **面向字节流**：TCP协议是面向字节流的协议，它不保留消息边界，但通过序列号保证数据的按序到达和完整性。
- 首部开销较大(20字节)
上面提到TCP协议建立在可靠的连接上，这就涉及到了TCP的连接过程，简单概述为三次握手，四次挥手。

### TCP三次握手

1. **第一次握手**：建立连接时，客户端发送一个SYN包到服务器，并进入SYN_SENT状态。
2. **第二次握手**：服务器收到SYN包，必须确认客户的SYN（acknowledgement number）包，同时自己也发送一个SYN包，即SYN+ACK包，此时服务器进入SYN_RCVD状态。
...
| 头部开销 | 较大(至少约 20 字节)    | 较小(8 字节)       |
| 使用场景 | 需要可靠传输的应用      | 更在意时延/吞吐的应用 |
| 控制机制 | 流量控制、拥塞控制      | 无（应用层自理）   |

TCP 常见于需要可靠字节流的场景；浏览网页时**应用层是 HTTP(S)**，传输层往往是 TCP（HTTP/3 则基于 QUIC/UDP）。

UDP 常见于实时音视频、部分游戏同步、DNS 查询等——更在意时延或由应用层自建可靠性。
