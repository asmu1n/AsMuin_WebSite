...
- **UDP：用户数据报协议，无连接的传输层协议，提供不可靠的数据传输。**
- IP：网际协议，用于实现网络间的通信。
...
- **二级域名**：紧跟着顶级域名```.baidu```
- **子域名 / 主机名**：```www``` 位于 `baidu.com` 之前，用于区分服务入口

## 分层与协议对照（简表）

| TCP/IP 层 | 大致对应 OSI | 前端常打交道的 |
| --- | --- | --- |
| 应用层 | 5–7 | HTTP(S)、DNS、WebSocket |
| 传输层 | 4 | TCP、UDP（HTTP/3 基于 QUIC/UDP） |
| 网际层 | 3 | IP、ICMP |
| 网络接口层 | 1–2 | 以太网、Wi‑Fi 等（浏览器通常不直接操作） |
