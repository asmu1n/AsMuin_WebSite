...
## `Prettier`和`ESLint`
`Prettier`和`ESLint`都是约束代码规范的工具，但它们的侧重点不一样。`ESlint`是能够去分析代码，从而去检查某些潜在的问题。但`Prettier`则是对代码进行格式化，不参与代码检查。 如果想检查不符合`Prettier`规则的信息，则需要通过插件将`Prettier`的规则录入`ESLint`去进行检查。
