...
一句话：**`slice` 可以先 `nil` 再 `append`；`map` 写入前一般要先 `make`。**  
补充：`nil slice` 不能按下标读写；`nil map` **读**返回零值不 panic，**写**会 panic。
...
结构体赋值是**字段级拷贝**。字段都是基本类型时，这就是一份独立副本。

若字段里有 `slice` / `map` / 指针，拷贝的是**头部或地址**，底层数据仍共享（浅拷贝）：
...
- 函数内优先 `:=`，包级变量用 `var`
- 先想零值能不能直接用
- `nil slice` 适合 `append`；`nil map` 写入前先 `make`
- 结构体初始化优先具名字段；赋值是浅拷贝
- 所谓字段展开，本质是嵌入后的字段提升
